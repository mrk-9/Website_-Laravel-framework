...
class StoreAdPlacementRequest extends Request {
	...
	public function rules()
	{
		return [
			'name' => ['required'],
			'price' => ['sometimes', 'required', 'regex:/^[1-9]\d*((\.|,)\d+)?$/'],
			// Sadly required_without only accept fields and not field values.
			'minimum_price' => [
				'sometimes',
				'required_if:type,' . AdPlacement::TYPE_HYBRID,
				'regex:/^[1-9]\d*((\.|,)\d+)?$/',
				'less_than:price'
			],
	...
	public function messages()
	{
		return [
			...
			'media_id.required' => 'Veuillez saisir le média associé. Si aucun n\'existe vous devez en créer un.',
